...
sortdialog_ui_old = """                if (showFolderSort) {
...
sortdialog_ui_new = """                if (showFolderSort) {
                    Text(folderSortTitle, style = MaterialTheme.typography.labelMedium)
                    if (showMediaSort) {
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            FilterChip(selected = folderSortBy == "name", onClick = { folderSortBy = "name" }, label = { Text("Name") })
                        ...
                        // Force name for albums and persons
                        folderSortBy = "name"
                    }
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        FilterChip(selected = folderSortDir == "asc", onClick = { folderSortDir = "asc" }, label = { Text("Aufsteigend") })
...
if sortdialog_ui_old in content:
    content = content.replace(sortdialog_ui_old, sortdialog_ui_new)
    print("Replaced sortdialog UI successfully")
else:
    print("Could not find sortdialog_ui_old")
